...
try:
    with engine.connect() as conn:
        ...
        print(f"Checking amortization settings for company_id: {company_id}")
        print("=" * 80)
        ...
        print(f"Use mark-based amortization: {use_mark_based}")
        ...
        for row in txn_result:
            ...
            total_txn_amort += current_year_amort

        print(f"\nFound {count} transactions with amortization settings")
        print(f"Total transaction amortization: Rp {total_txn_amort:,.2f}")

        # Get amortization_assets
        assets_query = text("""
            SELECT
        ...
        for row in assets_result:
            ...
            if a_start_date.year == year:
                ...
                if months > 12:
                    months = 12
                annual_amort = annual_amort * (months / 12)

            total_asset_amort += annual_amort

        print(f"Total asset amortization: Rp {total_asset_amort:,.2f}")

        grand_total = 52642290.42 + total_txn_amort + total_asset_amort
