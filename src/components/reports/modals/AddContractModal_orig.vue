...
// Debug: Reactive monitoring
watch(locations, (newLocations) => {
  console.log('Locations reactive update:', newLocations);
}, { deep: true });

// Test direct data assignment
const testLocations = () => {
  console.log('Testing locations directly...');
  locations.value = [
    { id: 'test1', location_name: 'Test Location 1', address: 'Test Address 1' },
    { id: 'test2', location_name: 'Test Location 2', address: 'Test Address 2' }
  ];
  console.log('Test locations assigned:', locations.value);
};
  ...
  if (isOpen) {
    ...
    try {
      ...
      console.log('Data loaded:', {
        ...
        transactions: linkableTransactions.value.length
      });
...
const handleSubmit = async () => {
  loading.value = true;
  try {
    const { payment_schedule, ...formDataWithoutSchedule } = form.value;
    const data = {
...
onMounted(async () => {
  console.log('Component mounted, isOpen:', props.isOpen);
  if (props.isOpen) {
    await init();
  }
});
