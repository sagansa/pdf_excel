<template>
  <div class="space-y-6">
    <div class="bg-white rounded-xl border border-gray-200 p-5">
      <h3 class="text-lg font-semibold text-gray-900">Service Transactions & Tax</h3>
      <p class="text-sm text-gray-500 mt-1">
        Mark jasa diatur dari menu Mark. Di sini tiap transaksi jasa punya konfigurasi pajak sendiri: NPWP, metode bruto/netto, dan waktu bayar pajak.
      </p>
    </div>
      ...
      <div class="flex flex-col lg:flex-row gap-3 lg:items-center lg:justify-between">
        <div class="grid grid-cols-1 md:grid-cols-3 gap-3 w-full lg:w-auto">
          <div class="bg-indigo-50 border border-indigo-100 rounded-lg px-3 py-2">
            <div class="text-xs uppercase font-semibold text-indigo-600">Total Transaksi</div>
            <div class="text-lg font-bold text-indigo-900">{{ summary.total_txn }}</div>
          </div>
          <div class="bg-emerald-50 border border-emerald-100 rounded-lg px-3 py-2">
            <div class="text-xs uppercase font-semibold text-emerald-600">Ada NPWP</div>
            <div class="text-lg font-bold text-emerald-900">{{ summary.with_npwp }}</div>
          </div>
          <div class="bg-amber-50 border border-amber-100 rounded-lg px-3 py-2">
            <div class="text-xs uppercase font-semibold text-amber-700">Preview Total Pajak</div>
            <div class="text-lg font-bold text-amber-900">{{ formatCurrency(summary.total_tax) }}</div>
          </div>
        </div>
    ...
    <div class="bg-white rounded-xl border border-gray-200 overflow-hidden">
      <div class="px-4 py-3 border-b border-gray-200 bg-gray-50">
        <h4 class="text-sm font-semibold text-gray-800">Service Transactions List</h4>
      </div>
      <div class="max-h-[560px] overflow-auto">
        <table class="min-w-full divide-y divide-gray-200 text-sm">
          <thead class="bg-white sticky top-0">
            <tr>
              <th class="px-4 py-2 text-left text-xs font-semibold text-gray-500 uppercase">Tanggal / Deskripsi</th>
              <th class="px-4 py-2 text-right text-xs font-semibold text-gray-500 uppercase">Nominal</th>
              <th class="px-4 py-2 text-left text-xs font-semibold text-gray-500 uppercase">Mark</th>
              <th class="px-4 py-2 text-left text-xs font-semibold text-gray-500 uppercase">NPWP</th>
              <th class="px-4 py-2 text-left text-xs font-semibold text-gray-500 uppercase">Metode</th>
              <th class="px-4 py-2 text-left text-xs font-semibold text-gray-500 uppercase">Waktu Bayar</th>
              <th class="px-4 py-2 text-left text-xs font-semibold text-gray-500 uppercase">Tgl Bayar</th>
              <th class="px-4 py-2 text-left text-xs font-semibold text-gray-500 uppercase">Preview Pajak</th>
              <th class="px-4 py-2 text-right text-xs font-semibold text-gray-500 uppercase">Aksi</th>
            </tr>
          </thead>
          <tbody class="divide-y divide-gray-100">
            <tr v-if="loadingTransactions">
              <td colspan="9" class="px-4 py-6 text-center text-gray-500">Loading transaksi jasa...</td>
            </tr>
            ...
            <tr v-for="txn in transactions" :key="txn.id" class="hover:bg-gray-50">
              <td class="px-4 py-3">
                ...
                <div class="text-xs text-gray-600 max-w-[300px] truncate">{{ txn.description || '-' }}</div>
              </td>
              <td class="px-4 py-3 text-right font-medium text-gray-900">{{ formatCurrency(txn.amount) }}</td>
              <td class="px-4 py-3 text-gray-700">{{ txn.personal_use || txn.internal_report || '-' }}</td>
              <td class="px-4 py-3">
                <span
                  v-if="txn.has_npwp"
              ...
              <td class="px-4 py-3 text-gray-700">{{ txn.service_calculation_method }}</td>
              <td class="px-4 py-3 text-gray-700">{{ formatTiming(txn.service_tax_payment_timing) }}</td>
              <td class="px-4 py-3 text-gray-700">{{ txn.service_tax_payment_date ? formatDate(txn.service_tax_payment_date) : '-' }}</td>
              <td class="px-4 py-3">
                <div class="text-xs text-gray-700">
                  <div>Tarif: <span class="font-semibold">{{ getPreview(txn).rate }}%</span></div>
                  <div>PPh: <span class="font-semibold">{{ formatCurrency(getPreview(txn).tax) }}</span></div>
                </div>
              </td>
              <td class="px-4 py-3 text-right">
                <button
                  @click="openEditor(txn)"
                  class="px-3 py-1.5 text-xs font-medium text-white bg-indigo-600 rounded-lg hover:bg-indigo-700"
                >
                  Edit Pajak
                </button>
              </td>
    ...
    >
      <div class="bg-white rounded-lg shadow-xl max-w-2xl w-full mx-4 max-h-[90vh] overflow-y-auto">
        <div class="p-6 border-b border-gray-200 flex items-center justify-between">
          <div>
            <h3 class="text-lg font-semibold text-gray-900">Edit Service Tax</h3>
            <p class="text-xs text-gray-500 mt-1">{{ formatDate(selectedTxn.txn_date) }} | {{ selectedTxn.description || '-' }}</p>
          </div>
        ...
        <div class="p-6 space-y-4">
          <div class="bg-gray-50 rounded-lg border border-gray-200 p-4">
            ...
            <div class="text-2xl font-bold text-gray-900">{{ formatCurrency(selectedTxn.amount) }}</div>
          </div>

          <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              <label class="block text-sm font-medium text-gray-700 mb-1">Status NPWP</label>
              <select
          ...
          <div class="grid grid-cols-1 md:grid-cols-3 gap-4 p-4 bg-indigo-50 border border-indigo-100 rounded-lg">
            ...
            <div>
              <label class="block text-sm font-medium text-gray-700 mb-1">Waktu Bayar Pajak</label>
              <select
            ...
            <div>
              <label class="block text-sm font-medium text-gray-700 mb-1">Tanggal Bayar Pajak</label>
              <input
          ...
          <div class="bg-white border border-gray-200 rounded-lg p-4">
            <div class="text-xs font-bold text-gray-500 uppercase mb-3">Preview Pajak Transaksi</div>
            <div class="grid grid-cols-2 md:grid-cols-4 gap-3 text-sm">
              <div>
                <div class="text-gray-500 text-xs">Tarif</div>
                <div class="font-semibold text-gray-900">{{ editorPreview.rate }}%</div>
              </div>
              <div>
                <div class="text-gray-500 text-xs">Nilai Bruto</div>
                <div class="font-semibold text-indigo-700">{{ formatCurrency(editorPreview.bruto) }}</div>
              </div>
              <div>
                <div class="text-gray-500 text-xs">Nilai Netto</div>
                <div class="font-semibold text-gray-900">{{ formatCurrency(editorPreview.netto) }}</div>
              </div>
              <div>
                <div class="text-gray-500 text-xs">PPh</div>
                <div class="font-semibold text-amber-700">{{ formatCurrency(editorPreview.tax) }}</div>
              </div>
            </div>
          </div>
        </div>
...
const normalizeMethod = (value) => {
  ...
  return method === 'NETTO' ? 'NETTO' : 'BRUTO';
};
...
const getPreviewByConfig = (amount, hasNpwp, method) => {
  ...
  if (normalizeMethod(method) === 'NETTO') {
...
const summary = computed(() => {
  ...
  const totalTax = transactions.value.reduce((acc, txn) => acc + Number(getPreview(txn).tax || 0), 0);
  return {
    total_txn: totalTxn,
    with_npwp: withNpwp,
    total_tax: totalTax
...
const loadServiceTransactions = async () => {
  ...
  try {
    const response = await historyApi.getServiceTransactions(props.companyId, props.year, search.value);
...
const saveEditor = async () => {
  ...
  try {
    ...
    const response = await historyApi.updateServiceTransactionTax(selectedTxn.value.id, payload);
    const updated = {
      ...selectedTxn.value,
      service_npwp: response.data.service_npwp || null,
      has_npwp: Boolean(response.data.has_npwp),
      service_calculation_method: normalizeMethod(response.data.service_calculation_method || payload.calculation_method),
      service_tax_payment_timing: normalizeTiming(response.data.service_tax_payment_timing || payload.tax_payment_timing),
      service_tax_payment_date: response.data.service_tax_payment_date || payload.tax_payment_date || null
    };

    const idx = transactions.value.findIndex((txn) => txn.id === selectedTxn.value.id);
    if (idx >= 0) {
    ...
    alert('Gagal simpan konfigurasi pajak jasa: ' + (error.response?.data?.error || error.message));
  } finally {
    editorSaving.value = false;
  }
};
